...
import { LAYER_COLORS } from '@/lib/network/types';
...
export function LayerToggle() {
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <RadioGroup
        value={activeLayer}
        onValueChange={(v) => setActiveLayer(v as NetworkLayer)}
        className="flex rounded-full bg-muted/50 p-1 gap-1"
      >
        {LAYERS.map((l) => {
          ...
          return (
            <RadioGroupItem
              ...
              aria-label={l.label}
              className={cn(
                'h-8 w-auto rounded-full px-3 text-xs font-medium border-transparent',
