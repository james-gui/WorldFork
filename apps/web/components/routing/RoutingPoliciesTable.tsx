...
const MODELS = ['openai/gpt-4o', 'openai/gpt-4o-mini', 'anthropic/claude-3-5-sonnet', 'anthropic/claude-3-haiku'];
...
const DEFAULT_ROWS: PolicyRow[] = JOB_TYPES.map((jt) => ({
  jobType: jt,
  provider: 'openrouter',
  model: jt === 'god_review' || jt === 'initializer' ? 'openai/gpt-4o' : 'openai/gpt-4o-mini',
  temperature: jt === 'initializer' ? 0.8 : 0.7,
  topP: 1,
  maxTokens: jt === 'god_review' ? 8192 : 4096,
  concurrency: jt === 'cohort_decision' ? 20 : 4,
  rpm: 500,
  tpm: 200000,
  dailyCap: 5000000,
}));
...
export function RoutingPoliciesTable() {
  const [data, setData] = React.useState<PolicyRow[]>(DEFAULT_ROWS);

  const updateRow = (jobType: JobType, field: keyof PolicyRow, value: string | number) => {
    setData((prev) =>
