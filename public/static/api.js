...
(function () {
  ...
  // In read-only static mode (window.WF_STATIC === true) all API URLs are
  // rewritten to flat JSON files baked into the deploy. /api/start is
  // refused outright — there's no orchestrator to talk to.
  function staticUrlFor(url) {
    if (url === "/api/runs") return "/data/runs.json";
    let m;
    if ((m = url.match(/^\/api\/run\/([^/]+)\/lineage$/))) {
      return `/data/lineage/${decodeURIComponent(m[1])}.json`;
    }
    if ((m = url.match(/^\/api\/run\/([^/]+)\/graph\?sim=(.+)$/))) {
      return `/data/graph/${decodeURIComponent(m[1])}/${decodeURIComponent(m[2])}.json`;
    }
    return null;
  }
  ...
  async function jget(url) {
    if (window.WF_STATIC) {
      ...
      if (!staticUrl) throw new Error(`no static mapping for ${url}`);
      url = staticUrl;
    }
  ...
  async function jpost(url, body) {
    if (window.WF_STATIC) {
      throw new Error("read-only demo — fresh runs disabled");
    }
